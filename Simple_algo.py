...
Joueurs["Name"] = "Denis", "Romain", "Mouhsin", "Paul", "Coco", "Theo", "Fabien", "Ben", "Erwan", "PierreHub"
Joueurs["Attaque"] = 55, 42, 69, 70, 40, 48, 50, 52, 65, 60
Joueurs["Defense"] = 65, 73, 64, 62, 60, 62, 60, 62, 68, 58
Joueurs["Technique"] = 45, 20, 90, 80, 20, 38, 30, 40, 40, 50
Joueurs["Rapidite"] = 58, 56, 55, 88, 50, 60, 50, 53, 70, 85
Joueurs["Lecture jeu"] = 65, 69, 88, 75, 45, 59, 60, 61, 69, 60
Joueurs["Endurance"] = 62, 65, 60, 70, 60, 65, 65, 61, 65, 75
Joueurs["Total"] = Joueurs.sum(axis=1)    
Equipes = pd.DataFrame(columns = ["Team_A_Score", "Team_B_Score", "Raw_score_difference"])
#%%Create random teams
...
for t in teams_list[:int(len(teams_list)/2)]:
    ...
    sum_A = Joueurs["Total"].iloc[list(team_A)].sum()
    sum_B = Joueurs["Total"].iloc[list(team_B)].sum()
#    print(team_A, A, team_B)
    Equipes = Equipes.append({"Team_A_Score":sum_A, "Team_B_Score":sum_B, "Raw_score_difference":abs(sum_B-sum_A)}, ignore_index=True)
...
team_A_names = np.array(Joueurs["Name"].iloc[list(team_A)])
team_B_names = np.array(Joueurs["Name"].iloc[list(team_B)])

print("L'opposition la plus équilibrée est:\n Equipe A : {0}\n Equipe B:{1}".format(team_A_names, team_B_names))

print(Joueurs.iloc[list(team_A)])
print(Joueurs.iloc[list(team_B)])
